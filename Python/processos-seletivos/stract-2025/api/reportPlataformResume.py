...
def report_platform_resume( access_token, platform):
    ...
    for item in accounts:
        ...
        numeric_values = {field: 0 for field in fields}
        for insight in insights:
            for field in fields:
                value = insight.get(field, 0)
                if isinstance(value, (int, float)):  # Verifica se é numérico
                    numeric_values[field] += value

        insight_values = [numeric_values.get(field, "") if isinstance(numeric_values.get(field), (int, float)) else " " for field in fields
        ]
